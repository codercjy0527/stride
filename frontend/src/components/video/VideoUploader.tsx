...
export default function VideoUploader({ onUpload, analyzing, barefoot }: { onUpload: (file: File) => void; analyzing: boolean; barefoot?: boolean }) {
  ...
  return (
    <div onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}
      ...
      <p className="text-[#8A94A6] font-medium">
        {analyzing ? '分析中...' : barefoot ? '点击或拖拽上传赤足跑步视频' : '点击或拖拽上传跑步视频'}
      </p>
      <p className="text-sm text-[#5A6080] mt-1">
        {barefoot ? '脱鞋后在相同角度拍摄，10-15秒即可' : '支持 MP4、MOV、AVI 格式'}
      </p>
